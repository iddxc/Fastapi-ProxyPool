...
这里连接字符串的格式需要符合 redis://[:password@]host[:port][/database] 的格式， 中括号参数可以省略，port默认是6379，database默认是0，密码默认为空。
...
APP_PROD_METHOD: 正式环境启动应用方式，默认是gevent， 可选：tornado，meinheld（分别需要安装tornado或meinheld模块）
Redis 连接
